...
The `xattr` step is required because VocaMac Lite isn't signed with an Apple Developer ID or notarized (releases are signed with a stable self-signed certificate instead — see Security notes in the main [README](https://github.com/vajahath/vocamac-lite#readme) — but Gatekeeper only trusts notarized apps). Removing the quarantine flag lets macOS launch it anyway. You can also right-click the app in Finder and choose Open.
